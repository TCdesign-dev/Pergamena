...
export function SchedaLettura({ quaderno }: { quaderno: Quaderno }) {
  ...
  return (
    <dl className={s.campiScheda}>
      ...
      {quaderno.docente && (<><dt>Docente</dt><dd>{quaderno.docente}</dd></>)}
      {quaderno.email && (<><dt>Email</dt><dd><a href={`mailto:${quaderno.email}`}>{quaderno.email}</a></dd></>)}
      {quaderno.ricevimento && (<><dt>Ricevimento</dt><dd>{quaderno.ricevimento}</dd></>)}
      {link.length > 0 && (
        <>
          <dt>Link</dt>
